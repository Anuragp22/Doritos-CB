...
export default function Citations({ sources, className, variant = 'chip' }) {
  ...
  if (variant === 'footnote') {
    return (
      <>
        <div className={cn('dispatch-footnotes', className)}>
          <div className="dispatch-footnotes__label">References &nbsp;·&nbsp; Filed</div>
          {sources.map((source) => (
            <button
              key={`${source.documentId}-${source.index}`}
              type="button"
              onClick={() => setActive(source)}
              className="dispatch-footnote"
            >
              <span className="dispatch-footnote__num">
                [{String(source.index).padStart(2, '0')}]
              </span>
              <span>
                <span className="dispatch-footnote__file">{source.filename}</span>
                {typeof source.score === 'number' && (
                  <span className="dispatch-footnote__score">
                    p={source.score.toFixed(3)}
                  </span>
                )}
                {source.snippet && (
                  <span className="dispatch-footnote__excerpt">
